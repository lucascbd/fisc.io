                                            ...
                                            <details class="group">
                                                ...
                                                <div class="p-4 space-y-3 bg-gray-100">
                                                    ${[...expArr].sort((a,b) => (b.date||b.original_date||"").localeCompare(a.date||a.original_date||"")).map(exp => {
                                                        ...
                                                        return '<div class="border rounded-lg p-3"><div class="flex justify-between items-start mb-2"><div class="flex-1 min-w-0"><div class="flex items-center gap-2 min-w-0"><p class="font-medium min-w-0" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + dn + '</p><button onclick="editExpense(' + exp.id + ')" style="flex-shrink:0;background:none;border:none;cursor:pointer;padding:0;font-size:13px;opacity:0.7;">✏️</button></div><p class="text-xs text-gray-500">' + new Date(exp.date + 'T12:00:00').toLocaleDateString('pt-BR') + '</p><p class="text-xs text-gray-500">⚖️ ' + exp.profile_name + '</p></div><div class="text-right ml-4" style="flex-shrink:0;white-space:nowrap;"><p class="font-bold">R$ ' + formatBRL(pv) + '</p>' + (expPaid > 0 ? '<p class="text-xs text-blue-600">Pagou: R$ ' + formatBRL(expPaid) + '</p>' : '') + '<p class="text-xs font-bold ' + (expBal >= 0 ? 'text-green-600' : 'text-red-600') + '">' + (expBal >= 0 ? '+' : '') + 'R$ ' + formatBRL(expBal) + '</p></div></div>' + instSection + '</div>';
                                                    }).join('')}
                                                </div>
                                            </details>
                                        </div>`;
